...
int clutch (double t, const double x[], double dxdt[], void * params){
   ...
   */
  s->md.force_clutch =   fclutch( x[ 2 ], ( x[ 1 ] - s->md.v_in ), s->md.clutch_damping );
...
int jac_clutch (double t, const double x[], double *dfdx, double dfdt[], void *params)
{
  ...
  gsl_matrix_set (J, 1, 2, -fclutch_dphi_derivative( x[ 2 ], x[ 1 ] - s->md.v_in)  / s->md.mass );
