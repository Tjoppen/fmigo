...
# Wrapping based on a compiled FMU
for d in $FMUWRAPPERS
do
    echo "add_subdirectory(meWrapperFmu/$d)" >> CMakeLists.txt
    GSL="-l cgsl,wrapperlib,m,fmilib"
    mkdir -p meWrapperFmu/${d}
    SRCFMU=$(pwd)/me/${d}/${d}.fmu
    DSTXML=$(pwd)/meWrapperFmu/${d}/modelDescription.xml
    PREFIX=wrappedfmu_
    pushd meWrapperFmu/$d
        mkdir -p sources
        python ${XML2WRAPPEDXML} -f ${SRCFMU} -p ${PREFIX} > ${DSTXML}
        python ${MD2HDR} -w ${DSTXML} > sources/modelDescription.h
        python ${CMAKE_GENERATOR} ${GSL} -t ${WRAPPERSOURCES} -f ${SRCFMU} -p ${PREFIX} -X ${XML2WRAPPEDXML}
    popd
done
