...
testit() {
  ...
  python ../../../compare_csv.py ref.csv ${s}.csv
  rm ${s}.csv
  echo "OK"
}

# Test the three different ways to get directional derivatives in wrapper.c
testit springs2 "pass-through (providesDirectionalDerivatives = true)"
testit springs3 "directional.txt"
testit springs4 "numerical derivative"
