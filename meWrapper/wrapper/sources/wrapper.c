...
void wrapper_init(state_t *s) {
  ...
  if (wrapper.m_version == fmi_version_2_0_enu) { // FMI 2.0
    ...
    wrapper.m_fmi2CallbackFunctions.logger = fmi2_log_forwarding;
    wrapper.m_fmi2CallbackFunctions.allocateMemory = calloc;
    wrapper.m_fmi2CallbackFunctions.freeMemory = free;
    wrapper.m_fmi2CallbackFunctions.stepFinished = 0;
    wrapper.m_fmi2CallbackFunctions.componentEnvironment = 0;

    // Load the binary (dll/so)
    jm_status_enu_t status = fmi2_import_create_dllfmu(wrapper.m_fmi2Instance, fmuType, &wrapper.m_fmi2CallbackFunctions);
