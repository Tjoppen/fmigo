...
test ${FMUS_DIR}/meWrapper/spring/wrapper_springs.fmu       tests/springs.csv      2
...
REStoCHECK="set"
test ${FMUS_DIR}/meWrapper/${SPRING2}     tests/springs2.csv     2 "${conn}"
REStoCHECK=""

conn="${conn} -p 1,v1,10:1,k_internal,1:1,k1,1"
conn="${conn} -C shaft,0,1,omega1,phi1,a1,f1,omega2,phi2,a2,f2"
test "${FMUS_DIR}/meWrapper/${SPRING2} ${FMUS_DIR}/meWrapper/${SPRING2}"          tests/connectedsprings.csv 3 "${conn}"
