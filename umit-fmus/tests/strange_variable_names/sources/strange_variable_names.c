...
static fmi2Status strange_variable_names_init(ModelInstance *comp) {
    ...
    strlcpy(s->md.space_in,  s->md.space_0, sizeof(s->md.space_in));
    strlcpy(s->md.space_out, s->md.space_0, sizeof(s->md.space_out));
    return fmi2OK;
}
...
static void doStep(state_t *s, fmi2Real currentCommunicationPoint, fmi2Real communicationStepSize, fmi2Boolean noSetFMUStatePriorToCurrentPoint) {
    ...
    strlcpy(s->md.space_out, s->md.space_in, sizeof(s->md.space_out));
}
